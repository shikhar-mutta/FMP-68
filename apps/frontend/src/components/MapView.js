...
const MapView = ({
  ...
  // Only auto-follow to live GPS when the user is actively tracking.
  // Without this guard, the map would always snap to currentPosition
  // (fetched on page load) even when not recording.
  const followPosition =
    role === 'publisher'
      ? publisherLatest
      : followerLatest || (isActivelyTracking ? currentPosition : null);
    ...
    <div className="map-container" id="map-view">
      ...
      >
        ...
        {followerLatest ? (
          ...
          /* No active follower path yet — show current GPS location as a
             green dot whenever autoFollow is on and we have a GPS fix. */
          role === 'follower' && autoFollow && currentPosition && (
            <>
              <Circle
