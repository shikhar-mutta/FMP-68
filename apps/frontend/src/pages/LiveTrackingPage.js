...
export default function LiveTrackingPage() {
  ...
  const [followerTripPaused, setFollowerTripPaused] = useState(false);
  ...
  // ── Publisher: Republish (reset + restart) ────────────────
  // Clears the old recorded path on the backend then starts fresh.
  const handleRepublishPath = useCallback(async () => {
    if (!user?.id) return;
    try {
      // 1. Reset on backend (clears coordinates, status → idle)
      await apiClient.post(`/paths/${pathId}/reset`);
      ...
      // 3. Kick off a fresh recording session immediately
      await startTracking(pathId, user?.id);
      setTrackingStatus('recording');
      setStartTime(Date.now());
      stopWatchRef.current = startGpsWatch('publisher');

      if (window.showToast) window.showToast('🔄 Path reset — republishing!', 'success');
    } catch (err) {
      console.error('Error republishing path:', err);
      if (window.showToast) window.showToast('Failed to republish path', 'error');
    }
  }, [pathId, user?.id, startGpsWatch]);
  ...
  // ── Follower: End Trip (no unfollow) ─────────────────────
  // Used when the publisher has already ended the session.
  // We just stop GPS and go home — no need to explicitly unfollow.
  ...
  // ── Follower: Pause Trip ─────────────────────────────
  // Pauses local GPS tracking without leaving or unfollowing.
  const handlePauseTrip = useCallback(() => {
    stopWatching();
    setCurrentSpeed(0);
    setFollowerTripPaused(true);
    if (window.showToast) window.showToast('⏸️ Trip paused', 'warning');
  }, []);

  // ── Follower: Resume Trip ───────────────────────────
  // Restarts GPS watch as follower after a pause.
  const handleResumeTrip = useCallback(() => {
    stopWatchRef.current = startGpsWatch('follower');
    setFollowerTripPaused(false);
        ...
        <div className="tracking-content">
          ...
          <div className="tracking-map-wrapper">
            <MapView
              ...
              isActivelyTracking={
                isPublisher
                  ? publisherCoords.length > 0 && trackingStatus === 'recording'
                  ...
                  {trackingStatus === 'ended' && (
                    <div className="tracking-ended-info">
                      ...
                      <div className="control-group" style={{ marginTop: '12px' }}>
                        <button
                          className="btn-control btn-start"
                          onClick={handleRepublishPath}
                        >
                          <span className="btn-icon">🔄</span>
                          <span>Republish Path</span>
                        </button>
                        <button
                  ...
                  {(trackingStatus === 'recording' ||
                    ...
                    <div className="control-group">
                      {followerCoords.length === 0 ? (
                        ...
                        // Follower has paused their own GPS — show Resume + End Trip
                        <>
                          <button
                            className="btn-control btn-resume"
                        ...
                        // Follower is actively tracking — show Pause + End Trip
                        <>
                          <button
                            className="btn-control btn-pause"
