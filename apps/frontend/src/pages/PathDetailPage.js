...
const PathDetailPage = () => {
  ...
  // Check if the current user already sent a pending follow request
  useEffect(() => {
    if (!user?.id) return;
    getSentFollowRequests()
      .then((reqs) => {
        const already = reqs.some((r) => r.pathId === pathId);
        setRequestSent(already);
      })
      .catch(() => {});
  }, [pathId, user?.id]);
        ...
        <div className="path-detail-container">
          ...
          <div className="pd-header-card">
            <div className="pd-header-top">
              ...
              {/* Live tracking — only for publisher/follower */}
              {canLiveTrack && (
                <button
              ...
              {/* Follow button — for non-publisher, non-follower users */}
              {!isPublisher && !isFollower && (
                requestSent ? (
                  <span className="pd-request-sent-badge">📨 Request Sent</span>
                ) : (
                  <button
                    className="pd-follow-btn"
                    onClick={handleFollowRequest}
                    disabled={requestLoading}
                  >
                    {requestLoading ? '⏳ Sending...' : '➕ Follow Path'}
                  </button>
                )
              )}
            </div>
