...
{\Large \textbf{CSE 816 — Software Production Engineering}}\\[0.4cm]
{\large Final Project Report}\\[3cm]

{\Huge \textbf{FMP-68}}\\[0.4cm]
{\LARGE \textbf{A DevSecOps SDLC Automation Platform}}\\[0.2cm]
{\large with selective-rebuild CI/CD, container image scanning,\\
in-cluster secrets vaulting, and dashboards-as-code observability}\\[2cm]
...
\textbf{\Large Author}\\[0.3cm]
{\large Shikhar Mutta}\\[2cm]

\textbf{\Large Source Code Repository}\\[0.3cm]
\texttt{github.com/shikhar-mutta/FMP-68}\\[3cm]

\vfill
{\small Submitted in partial fulfilment of CSE 816 — Software Production Engineering}\\
{\small Final Project, Spring 2026}
\end{center}
\end{titlepage}
...
\textbf{FMP-68} is a full-stack microservices platform whose primary
contribution is not the application code itself but the \emph{DevSecOps
software-development-lifecycle (SDLC) automation} built around it.  The
...
\dummyfig{RabbitMQ management UI showing the
\texttt{fmp.notifications} exchange and the
\texttt{notification-service.consumer} queue.}
...
\dummyfig{Redis CLI output showing the \texttt{trail:*} keys with
TTLs, demonstrating the live-broadcast cache layer.}
...
\dummyfig{Docker Hub view of the \texttt{shikhar68/fmp-*} repository
list, showing the most recent build tags.}
...
\dummyfig{Jenkins dashboard showing the orchestrator and seven
per-service pipelines, with their last build status.}
...
\dummyfig{Output of \texttt{kubectl get pods -A} after a clean
\texttt{ansible-playbook} run, showing all pods in
Running / Ready state across the fmp, elk, and vault namespaces.}
...
\dummyfig{Vault UI showing the kv-v2 secrets engine mounted at
\texttt{secret/} with the \texttt{fmp/auth},
\texttt{fmp/db}, and \texttt{fmp/rabbitmq} paths.}
...
\dummyfig{Kibana Discover view filtered to
\texttt{service: paths-service}, showing structured log entries with
the \texttt{kubernetes.pod.name} and \texttt{message} fields
expanded.}
